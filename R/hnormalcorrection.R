hnormalcorrection <- function(p1, n1, p0, n0) {
  ...
  # Convertir incrementos a fracciones irreducibles
  frac1 <- .rat(p1 / n1)
  frac2 <- .rat(p0 / n0)
  
  # Extraer numeradores y denominadores
  num1 <- frac1[1]
  den1 <- frac1[2]
  num2 <- frac2[1]
  den2 <- frac2[2]

  # Calcular gcd(a, b) para enteros
  gcd_int <- function(a, b) {
    if (b == 0) abs(a) else gcd_int(b, a %% b)
  }
  
  # Calcular h = gcd(inc1, inc2) = gcd(num1*den2, num2*den1) / (den1*den2)
  a_int <- num1 * den2
  b_int <- num2 * den1
